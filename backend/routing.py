...
def create_app():
    ...
    def detect():
        ...
        return jsonify({
            'report_id': str(report['_id']),     # convert ObjectId → string
            'report': {
                **{k: v for k, v in report.items() if k != '_id'},  # all other fields
                '_id': str(report['_id'])        # convert _id inside the report too
            }
        }), 201 # frontend receives the report ID and report details in JSON format
...
if __name__ == "__main__":
    port = int(os.environ.get("PORT", 7860))  # HF exposes $PORT
